...
def build_cfg_path(feature_type: str) -> os.PathLike:
    ...
    '''
    path_base = Path('./sn-spotting/Utils/configs/')
...
def act_2_soccer(res):
    ...
    for f, segments in res.items():
        ...
        for segment in segments:
            conf = segment['score']
            if '1_224p' in f or '1_I3D' in f:
                game_string = "1 - "
                half = "1"
                tmp = "AA".join(f.split("AA")[:-1])+"AA2_224p"  
            elif '2_224p' in f or '2_I3D' in f:
                game_string = "2 - "
                half = "2"
        ...
        for segment in segments:
            conf = segment['score']
            if '1_224p' in other or '1_I3D' in f:
                game_string = "1 - "
                half = "1"
            elif '2_224p' in other or '2_I3D' in f:
                game_string = "2 - "
                half = "2"
            else:
                assert False
