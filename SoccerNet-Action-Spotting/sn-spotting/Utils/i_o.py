...
def reencode_video_with_diff_fps(video_path: str, tmp_path: str, extraction_fps: float) -> str:
    ...
    cmd1 = f'{which_ffmpeg()} -hide_banner -loglevel panic '
    cmd1 += f'-y -i'
    cmd1 = cmd1.split()
    # we need to handle spaces in the path in soccrenet, hence splitting command into two commands.
    cmd1.append(video_path)
    cmd2 = f'-filter:v fps=fps={extraction_fps} {new_path}'.split()
    cmd = cmd1 + cmd2
    subprocess.call(cmd)
    return new_path
